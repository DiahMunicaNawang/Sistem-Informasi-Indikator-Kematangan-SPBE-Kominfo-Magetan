                        ...
                        <div class="gap-2 d-flex">
                            @if ($forum_discussion->availability_status == 'open')
                                <a href="{{ route('forum-discussion-approval-accept', $forum_discussion->id) }}"
                                    class="btn btn-sm btn-light-danger">Tutup Diskusi</a>
                            @else
                                <a href="{{ route('forum-discussion-approval-accept', $forum_discussion->id) }}"
                                    class="btn btn-sm btn-light-success">Buka Diskusi</a>
                            @endif
                            <a href="{{ route('forum-discussion-approval-reject', $forum_discussion->id) }}"
                                class="btn btn-sm btn-light-danger"
                                onclick="return confirm('Apakah Anda yakin ingin menolak forum diskusi ini?')">Tolak</a>
                        </div>
                    </div>
